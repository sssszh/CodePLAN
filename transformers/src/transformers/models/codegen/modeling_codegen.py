...
class CodeGenForCausalLM(CodeGenPreTrainedModel):
    ...
    def __init__(self, config, tuning_mode=None, clone_pl_head=False):
        super().__init__(config)
        self.transformer = CodeGenModel(config)
        self.lm_head = nn.Linear(config.n_embd, config.vocab_size)
        self.clone_pl_head = clone_pl_head
        self.tuning_mode = tuning_mode

        if self.clone_pl_head: 
            # Optional: have a seperate linear layer for plan training 
            self.pl_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))*2
        ...
        if plan_id is not None:
            if self.clone_pl_head:
                pl_logits = self.pl_head(hidden_states).to(torch.float32)
            else:
                pl_logits = lm_logits
            shift_logits = pl_logits[..., :-1, :].contiguous()
            shift_labels = labels[..., 1:].contiguous()

            pl_loss_fct = CrossEntropyLoss()
            loss = pl_loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))

            pl_loss = loss.to(hidden_states.dtype)
